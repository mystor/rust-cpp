...
fn type_to_cpp_type<'tcx>(tcx: &ty::ctxt<'tcx>,
    ...
    match ty.sty {
        ...
        ty_ptr(ty::mt { ref ty, .. }) |
        ty_rptr(_, ty::mt { ref ty, .. }) |
        ty_uniq(ref ty) => {
            // We need to know if the type is Sized.
            // !Sized pointers are twice as wide as Sized pointers.
            if ty::type_is_sized(&ty::ParameterEnvironment::for_item(tcx, node_id), span, ty) {
                // If it is a sized type, then the width of the type is the same as a pointer.
                // So we can treat it like a C++ raw pointer.
                return format!("{}*", type_to_cpp_type(tcx, ty, span, node_id,
                                                       header, false, true));
            } else {
                // It's a trait object or slice!
                return match ty.sty {
                    ty_str => format!("Slice<uint8_t>"),
                    ty_vec(ref ty, None) => {
                        format!("Slice<{}>",
                                type_to_cpp_type(tcx, ty, span, node_id,
                                                 header, true, false))
                    }
                    _ => format!("TraitObject"),
                }
            }
        }
...
fn record_type_data(cx: &Context,
    ...
    if let Some(cppfn) = decls.get_mut(name) {
        ...
        cppfn.ret_ty = Some(type_to_cpp_type(cx.tcx, ret_ty, call.span, call.id,
                                             &mut headers, true, true));
                ...
                if let ExprCast(ref e, _) = e.node {
                    if let ExprAddrOf(_, ref e) = e.node {
                        ...
                        cppfn.arg_idents[i].ty =
                            Some(type_to_cpp_type(cx.tcx, arg_ty, e.span, e.id,
                                                  &mut headers, false, false));
                        continue
                    }
                }
            }
