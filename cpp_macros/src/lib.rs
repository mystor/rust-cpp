...
use cpp_common::{flags, parsing, FILE_HASH, LIB_NAME, MSVC_LIB_NAME, OUT_DIR, VERSION};
...
pub fn expand_internal(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let input = syn::buffer::TokenBuffer::new(tokens.parse().unwrap());
    let closure = parsing::cpp_closure(input.begin()).expect("cpp! macro").0;
...
NOTE: They cannot be generated by macro expansion."#,
    ...
    for (i, capture) in closure.sig.captures.iter().enumerate() {
        ...
        let mac_name = Ident::new(&format!("var_{}", written_name), Span::call_site());
        let mac_cty = Ident::new(&format!("cty_{}", written_name), Span::call_site());
        ...
        let arg_name = Ident::new(&format!("arg_{}", written_name), Span::call_site());
    ...
    let input = syn::buffer::TokenBuffer::new(closure.body.parse().unwrap());
    let rust_invocations = parsing::find_all_rust_macro(input.begin())
        .expect("rust! macro")
        .0;
...
pub fn expand_wrap_class(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let input = syn::buffer::TokenBuffer::new(tokens.parse().unwrap());
    let class = parsing::cpp_class(input.begin())
