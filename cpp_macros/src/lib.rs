...
use cpp_common::{flags, RustInvocation, FILE_HASH, LIB_NAME, MSVC_LIB_NAME, OUT_DIR, VERSION};
...
pub fn expand_internal(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let input: ::proc_macro2::TokenStream = tokens.parse().unwrap();
    let closure = ::syn::parse2::<cpp_common::Closure>(input).expect("cpp! macro");
...
pub fn expand_wrap_class(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let input: ::proc_macro2::TokenStream = tokens.parse().unwrap();
    let class = ::syn::parse2::<cpp_common::Class>(input).expect("cpp_class! macro");
