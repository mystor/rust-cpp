...
pub fn expand_internal(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let expr = &item_enum
        ...
        .unwrap()
        .1;
    let expr = unwrap_enum!(expr => syn::Expr::Field);
    let expr = unwrap_enum!(*expr.base => syn::Expr::Tuple);
    let expr = unwrap_enum!(expr.elems.first().unwrap().value().clone() => syn::Expr::Macro);

    let closure = match ::syn::parse2::<cpp_common::Closure>(expr.mac.tts.clone()) {
        Ok(x) => x,
...
pub fn expand_wrap_class(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let expr = unwrap_enum!(&item_struct.fields.iter().nth(0).unwrap().ty => syn::Type::Array);
    let expr = unwrap_enum!(expr.len => syn::Expr::Binary);
    let expr = unwrap_enum!(*expr.right => syn::Expr::Field);
    let expr = unwrap_enum!(*expr.base => syn::Expr::Tuple);
    let expr = unwrap_enum!(expr.elems.first().unwrap().value().clone() => syn::Expr::Macro);

    let class = match ::syn::parse2::<cpp_common::Class>(expr.mac.tts.clone()) {
        Ok(x) => x,
        Err(err) => return err.to_compile_error().into(),
    };
